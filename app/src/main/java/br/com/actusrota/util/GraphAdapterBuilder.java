...
public class GraphAdapterBuilder {
	...
	public GraphAdapterBuilder addType(Type type) {
		final ObjectConstructor<?> objectConstructor = constructorConstructor
				.get(TypeToken.get(type));
	...
	public void registerOn(GsonBuilder gsonBuilder) {
		...
		for (Map.Entry<Type, InstanceCreator<?>> entry : instanceCreators
				.entrySet()) {
	...
	static class Factory implements TypeAdapterFactory, InstanceCreator {
		...
		public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
			...
			final TypeAdapter<T> typeAdapter = gson.getDelegateAdapter(this,
					type);
			...
			return new TypeAdapter<T>() {
				...
				public void write(JsonWriter out, T value) throws IOException {
					...
					if (graph == null) {
						...
						graph = new Graph(
								new IdentityHashMap<Object, Element<?>>());
					}
					...
					if (element == null) {
						element = new Element<T>(value, graph.nextName(),
								typeAdapter, null);
				...
				public T read(JsonReader in) throws IOException {
					...
					if (graph == null) {
						...
						while (in.hasNext()) {
							...
							graph.map.put(name, new Element<T>(null, name,
									typeAdapter, element));
						}
						in.endObject();
					} else {
					...
					try {
						@SuppressWarnings("unchecked")
						// graph.map guarantees consistency between value and T
						Element<T> element = (Element<T>) graph.map
								.get(currentName);
		...
		public Object createInstance(Type type) {
			...
			if (graph == null || graph.nextCreate == null) {
				throw new IllegalStateException(
						"Unexpected call to createInstance() for " + type);
			}
	...
	static class Element<T> {
		...
		Element(T value, String id, TypeAdapter<T> typeAdapter,
				JsonElement element) {
			this.value = value;
			this.id = id;
		...
		void read(Graph graph) throws IOException {
			if (graph.nextCreate != null) {
				throw new IllegalStateException(
						"Unexpected recursive call to read() for " + id);
			}
			graph.nextCreate = this;
			value = typeAdapter.fromJsonTree(element);
			if (value == null) {
				throw new IllegalStateException(
						"non-null value deserialized to null: " + element);
			}
		}
	}
}
